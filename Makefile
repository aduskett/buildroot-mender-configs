...
help:
	...
	@printf "\tTARGET: Specify the target of which to run menuconfig. Default: x86\n"
	@printf "\n"
	@printf "Targets:\n"
	...
	@printf "\tmenuconfig: run menuconfig on a given target. Default: x86\n"
...
menuconfig:
	...
	docker exec -it buildroot-mender-configs-build /bin/bash -c 'cd ~/buildroot/external/output/${TARGET} && make menuconfig'
...
.PHONY: shell
shell:
	/bin/bash -c 'if [ -z $(docker container ps -aqf "name=buildroot-mender-configs-build") ]; then make APPLY_CONFIGS=${APPLY_CONFIGS} NO_BUILD=true EXIT_AFTER_BUILD=false upd; fi;'
	/bin/bash -c 'if [[ ${APPLY_CONFIGS} == "true" ]]; then sleep 5; fi'
	docker exec -it buildroot-mender-configs-build /bin/bash
